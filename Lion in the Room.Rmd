...
Creating retraaction-author-author network
```{r}
...
Generating list of unique authors
```{r}
authors_list_1 <- expanded_dataset %>%
  select(author) %>%
  unique()

write.csv(authors_list_1, "authors_list.csv")
```
...
url <- "https://scholar.google.com/citations?user=rq48Nj8AAAAJ&hl=en&oi=ao"

# Read the web page
page <- read_html(url)

# Extract author name and affiliation
author_name <- page %>% html_node("#gsc_prf_in") %>% html_text()
affiliation <- page %>% html_node(".gsc_prf_il") %>% html_text()
interests <- page %>% html_nodes(".gsc_prf_inta gs_ibl") %>% html_text()
total_citations <- page %>% html_node("#gsc_rsb_st td.gsc_rsb_std") %>% html_text()
co_authors <- page %>% html_nodes("#gsc_rsb_co a") %>% html_text()
education_details <- page %>% html_nodes("css-selector-for-education") %>% html_text()
total_papers <- page %>% html_nodes("#gsc_a_nn") %>% html_text()

# Print the results
print(paste("Author Name:", author_name))
print(paste("Affiliation:", affiliation))
print(paste("Interests:", paste(interests, collapse = ", ")))
print(paste("Total Citations:", total_citations))
print(paste("Co-Authors:", paste(co_authors, collapse = ", ")))
print(paste("Education Details:", paste(education_details, collapse = ", ")))
print(paste("Total Number of Papers:", total_papers))
